...
def test_apply_llm_settings_updates_config(tmp_path, monkeypatch):
    ...
    _write_config({"llm": {"base_url": "http://127.0.0.1:8000/v1", "model": "old"}})
...
def test_apply_llm_base_url(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _write_config({"llm": {"base_url": "http://127.0.0.1:8000/v1", "model": "bielik"}})
...
def test_current_llm_config_custom_url(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    _write_config({"llm": {"base_url": "https://api.example.com/v1", "model": "x"}})
