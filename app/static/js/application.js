    ...
    if (preflight) {
      if (!preflight.llm || !preflight.ready_for_draft) {
        ...
        if (st === "idle" || st === "starting") {
          const wake = preflight.wake_url ? preflight.wake_url.replace(/\/$/, "") + "/wake" : "http://192.168.0.112:8099/wake";
          msgs.push("LLM budzi się — odśwież za chwilę lub: curl -X POST " + wake);
        } else {
          msgs.push(
            "LLM niedostępny (sprawdź BC-250 :8006 / :8099) — bez LLM zostanie użyty baseline CV."
          );
        }
      }
