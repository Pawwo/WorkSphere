...
### Infrastruktura na `192.168.0.112`
...
- **SearXNG** — Docker na `192.168.0.194`, port `8888` (`deploy/searxng/`)

```bash
curl -s http://192.168.0.112:8006/v1/models
curl -s "http://192.168.0.194:8888/search?q=test&format=json" | head
```

**Wydajność (BC-250 Vulkan, Minitron-7B Q4_K_M, llama.cpp 9596, 2026-06-12):**
...
**Aplikacja (optymalizacja pod BC-250, `config.yaml`):**
...
| Usługa | Port | Rola |
|--------|------|------|
| `bc250-llm-manager` | **8099** | `POST /wake`, `POST /sleep`, `GET /status` |
| `bc250-llm-idle.timer` | — | auto-stop LLM po 50 min bezczynności |
...
**Produkcja (2026-06-12):** `llama-server-bielik.service` (Vulkan `-ngl 999`, port **8006**). Gate A/C PASS po formacie + coopmat2 build. Tuning: `scripts/bc250_llm_gate.sh`, skill `.cursor/skills/bc250-vulkan-tuning/`.

Po restarcie: binaria w `/usr/local` (build **9596** / `18ef86ece`, `deploy/bc250/build-llama-vulkan.sh --coopmat2`). Jina (`:8007`) na CPU (`-ngl 0`).

Szczegóły: [docs/bc250-llm-power-baseline.md](docs/bc250-llm-power-baseline.md), [ADR-007](docs/decisions/ADR-007-bc250-hybrid-power.md).
